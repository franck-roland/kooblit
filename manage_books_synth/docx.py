...
dic_head = {'Title': '<h1>','Heading1': '<h2>','Heading2': '<h3>','Heading3': '<h4>'}
def getStyle(element, context):
    val = element.attrib['{'+nsprefixes['w']+'}val']
    result = []
    if val != 'ListParagraph':
        if context['doing']:
            result = ['</ul>']
            context['doing'] = 0
            context['first'] = 1
        result.append(dic_head.get(val,''))
        return result

    elif val == 'ListParagraph':
        if context['first']:
            context['doing'] = 1
            context['first'] = 0
            return '<ul>','<li>',
...
def getdocumentHtml(document):
    '''Return the raw text of a document, as a list of paragraphs.'''
    paratextlist = []
    context = {'first': 1, 'doing': 0}
    ...
    for para in paralist:
        ...
        for element in para.iter():
            ...
            if element.tag == '{'+nsprefixes['w']+'}pStyle':
                print(element.attrib['{'+nsprefixes['w']+'}val'])
                paratext_list.extend(getStyle(element, context))
            elif element.tag == '{'+nsprefixes['w']+'}t':
                if element.text:
                    paratext_list.append(element.text)
            elif element.tag == '{'+nsprefixes['w']+'}tab':
                paratext_list.append('\t')
        if not set(paratext_list) & set(dic_head.values()):
            if len(paratextlist) == 1:
                paratext_list.insert(0,'<div class="summary">')
                paratext_list.append('</div>')
            else:
                paratext_list.insert(0,'<p>')
                paratext_list.append('</p>') 
        paratext_list.extend(close_elements(paratext_list))
        paratext = u''.join(paratext_list)
